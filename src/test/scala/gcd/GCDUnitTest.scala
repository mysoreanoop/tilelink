...
//    val top = Module(LazyModule(new LazyAXITop()(p)).module)
...
class TBLazyGCDTop(c:ChiselTopWrapper) extends PeekPokeTester(c) {
  println("Hello!!!")
  var cnt:BigInt = 0
  while(cnt <300) {
    cnt = cnt + 1
...
//FOR REDEFINE
  ...
  val io = IO(new Bundle{
    val red = r.io.cloneType
    val zynq = z.io.cloneType
  })
  io.red  <> r.io
  io.zynq <> z.io
  r.io.chiplink.b2c :=  z.io.chiplink.c2b
  z.io.chiplink.b2c :=  r.io.chiplink.c2b
